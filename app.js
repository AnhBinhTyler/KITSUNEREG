...
document.addEventListener("DOMContentLoaded",function(){
  ...
  function buildNotice(){
    const items=[
      {icon:"fa-gears",text:"AIMLOCK - KHOÁ TÂM BÁM ĐẦU CHẶT"},
      {icon:"fa-star",text:"FEATHER AIM - CẢI THIỆN NHẸ TÂM CAO"},
      {icon:"fa-bullseye",text:"STEADY HOLD - CẢI THIỆN ĐẦM TÂM KHI KÉO "},
      {icon:"fa-hand-pointer",text:"ANCHOR AIM - GHÌM TÂM KHI RA ĐẠN"},
      {icon:"fa-rocket",text:"SHAKE FIX - LOẠI BỎ HOÀN TOÀN RUNG TÂM KHI KÉO"}
    ];
  ...
  function openMenu(){menu&&menu.classList.remove("hidden"); menu&&menu.setAttribute("aria-hidden","false"); toast("DRAKONX TECH PRO 2 ĐÃ MỞ"); if(!store.get("menuNoticeSeen",false)){openNotice(); store.set("menuNoticeSeen",true)}}
  ...
  if(dpiSelect)    dpiSelect.addEventListener("change",()=>{store.set("dpi",dpiSelect.value); applyDpi(dpiSelect.value); toast("DPI FULER : "+dpiSelect.value+"x")});
  if(activateBtn)  activateBtn.addEventListener("click",()=>{
    const s=[
      ...
      "DPI FULER : "+(dpiSelect&&dpiSelect.value)+"x"
    ].join(" | ");
    toast("DRAKONX TECH ON | "+s); pulse(activateBtn);
  ...
  if(applyBtn)      applyBtn.addEventListener("click",()=>{store.set("feat-anti-shake",!!f1&&f1.checked);store.set("feat-aim-assist",!!f2&&f2.checked);store.set("feat-touch-boost",!!f3&&f3.checked);store.set("feat-pro-mode",!!f4&&f4.checked); toast("Đã Áp Dụng Tinh Chỉnh DrakonX Tech Pro 2"); closeMenu()});
